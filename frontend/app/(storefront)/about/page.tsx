...
type ArtisanRecord = Record<string, unknown>;

function getArtisanList(data: unknown): ArtisanRecord[] {
  if (Array.isArray(data)) {
    return data.filter(
      (item): item is ArtisanRecord =>
...
export default async function AboutPage() {
  ...
  try {
    ...
    artisanCount = getArtisanList(response).length;
  } catch {
    artisanCount = 0;
  }
  ...
  return (
    <main className="min-h-screen bg-[#f7f0df] text-[#3d241b]">
      ...
      <section className="mx-auto max-w-5xl px-6 py-20 text-center md:py-28">
        ...
        {artisanCount > 0 && (
          <div className="mx-auto mt-10 inline-flex items-center gap-3 border border-[#b08a45]/40 bg-[#efe5d0] px-6 py-3 text-sm text-[#6b5044]">
            <Users size={18} className="text-[#8b2635]" />
            <span>
              Discover the work of{" "}
              <strong className="font-semibold text-[#7b2532]">
                {artisanCount} artisan{artisanCount === 1 ? "" : "s"}
              </strong>{" "}
              on KALAKRITI.
            </span>
          </div>
